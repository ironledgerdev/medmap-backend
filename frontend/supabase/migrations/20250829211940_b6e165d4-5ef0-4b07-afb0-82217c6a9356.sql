-- Security improvements and database optimizations (fixed constraints)
...
-- 4. Add proper constraints (check if exists first)
DO $$
BEGIN
    -- Add appointment date constraint
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints 
                  WHERE constraint_name = 'check_appointment_date_future' 
    ...
    -- Add membership type constraint
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints 
                  WHERE constraint_name = 'check_membership_type' 
...
-- 6. Update handle_new_user function for proper role and data handling
